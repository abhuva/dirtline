...
def run(t):
    ...
    def select_zoom(level,exit_key=None):
        ...
        t.check(f'Map {s["map"]} settings retain correct minimap scale without regenerating map',
                after['mode']==1 and after['zoom_level']==level and after['radar_scale']==(128>>level) and
    ...
    t.step(t.DOWN,18); t.tap(t.A); s=t.step(0,45)
    ...
    menu(); alternate=t.start_map(1)
    t.check('Another catalog map retains the selected image zoom',alternate['zoom_level']==2 and alternate['radar_scale']==32,alternate)
    # Exercise every overview zoom while driving across full map reloads.
