import {compile, makeNode, presets} from './recipe.mjs';
...
const original = await fetch('/recipes/wasteland.json').then(r => r.json());
compile(original,schema); // Upgrade old road parameters before the first inspector render.
const examples = presets(schema, original);
const ops = new Map(schema.operations.map(o => [o.id,o]));
let recipe = clone(original), selected = recipe.output, pending = null;
...
try {
  const saved = JSON.parse(localStorage.getItem('dustline.recipe.v1'));
  if (saved) { compile(saved, schema); recipe = saved; selected = recipe.output; }
} catch { /* A broken/old draft must never prevent opening the workshop. */ }
...
function save() {
  try { localStorage.setItem('dustline.recipe.v1', JSON.stringify(recipe)); } catch { /* Export remains available. */ }
...
function restore(from, to) {
  ...
  if (!recipe.nodes.some(n => n.id === selected)) selected = recipe.output;
...
function renderGraph() {
  ...
  $('graph-plane').style.transform=`scale(${zoom})`;
  $('graph-stage').style.width=`${width*zoom}px`; $('graph-stage').style.height=`${height*zoom}px`;
  ...
  $('graph-hint').textContent=pending===null?'Drag between ports to connect or replace. Click × beside an input to disconnect.':'Choose any matching input to connect or replace. Escape cancels.';
}
...
function renderInspector() {
  ...
  if (!n) return;
  const op=ops.get(n.type), settings=element('div','settings');
  const label=element('input'); label.value=n.label; label.maxLength=80;
  label.onchange=()=>change(()=>{n.label=label.value;}); setting(settings,'Node label',label,true);
  for(const [port,expected] of Object.entries(op.inputs)) {
    ...
    setting(settings,`${port==='mask'?'Apply within mask':`Input ${port.toUpperCase()}`} · ${expected}`,input);
  }
  const parameterOrder=op.parameters.map((_,i)=>i);
  parameterOrder.forEach(index=>{
  ...
  if(n.type==='material_bands')settings.append(element('p','setting-help','Map field values into four material IDs. Boundaries must be ascending; values equal to a boundary enter the next band.'));
  if(n.type==='material_paint')settings.append(element('p','setting-help','Replace the material wherever the mask is 1. All other cells keep their input material.'));
  if(n.type==='materials'){
    settings.append(element('p','setting-help','One ground ID per 128 × 128 world-pixel cell. This output controls ground art and grip independently of wall/floor. Walls and settlement stamps retain their own art.'));
...
function generate() {
  ...
  try {
    ...
    const iterationNode=previewRecipe.nodes.find(n=>n.id===settingsNodeId());
    ...
    $('recipe-status').textContent=final?`${recipe.nodes.length} NODES · WALL/FLOOR + ${materialProgram?'GROUND IDS':'ORIGINAL GROUND'} · v${recipe.version}`:`${recipe.nodes.length} NODES · OUTPUT NEEDS A CONNECTION`;
    ...
    worker.postMessage({id:sequence,program:compiled.program,materialProgram,spawnProgram,decorationProgram,showSpawns:$('show-spawns').checked,seed:recipe.seed,collision:$('layer').value==='collision' && !seeds,textures:$('layer').value==='textures',seeds,compare});
  } catch(error) {
...
function paint(output,regionView=false,refined=true) {
...
function draw(data) {
  ...
  const legend=first.meta[1]===3 || first.texture ? materialCatalog.map(m=>[m.name,m.color]) : [['Floor','#d8b77b'],['Wall','#28483d'],['Spawn','#f08b5b'],['Outpost','#e7f6bc']];
  ...
  if(grid) {
    data.outputs.forEach((output,i)=>{
      ...
      ctx.drawImage(paint(output,layer==='regions',false),x,y,208,186);
      ctx.fillStyle='#ccd8bc';ctx.font='13px Consolas,monospace';ctx.fillText(String(output.seed),x+5,y+204);
    });
  } else if(data.before) {
    const before=paint(data.before,layer==='regions',false),after=paint(first,layer==='regions');
  ...
  } else {ctx.drawImage(paint(first,layer==='regions'),0,0,640,640);if(!first.texture)markers(ctx,first,0,0,640);}
...
examples.forEach((r,i)=>{const option=element('option','',r.name);option.value=i;$('preset').append(option);});
$('preset').onchange=()=>change(()=>{recipe=clone(examples[Number($('preset').value)]);selected=recipe.output;pending=null;pinnedSettings=null;arrange();$('view').value='final';});
...
schema.operations.forEach(op=>{
  ...
  button.onclick=()=>{
    ...
    change(()=>{
      const id=Math.max(...recipe.nodes.map(n=>n.id))+1,n=makeNode(schema,op.id,id,35+recipe.nodes.length%3*220,45+Math.floor(recipe.nodes.length/3)*190);
      for(const [port,expected] of Object.entries(op.inputs)) {
    ...
    $('graph').scrollTo({left:Math.max(0,recipe.nodes.at(-1).x*zoom-30),top:Math.max(0,recipe.nodes.at(-1).y*zoom-40),behavior:'smooth'});
  };
  $('library').append(button);
});
$('undo').onclick=()=>restore(undo,redo);$('redo').onclick=()=>restore(redo,undo);
$('arrange').onclick=()=>change(arrange);
function setZoom(value){zoom=Math.max(.25,Math.min(1.5,value));renderGraph();}
$('zoom-in').onclick=()=>setZoom(zoom+.1);$('zoom-out').onclick=()=>setZoom(zoom-.1);
$('fit-graph').onclick=()=>{
  const width=Math.max(...recipe.nodes.map(n=>(Number(n.x)||0)+230)),height=Math.max(...recipe.nodes.map(n=>(Number(n.y)||0)+210));
  setZoom(Math.min(1,($('graph').clientWidth-20)/width,($('graph').clientHeight-20)/height));$('graph').scrollTo(0,0);
};
$('graph').addEventListener('wheel',event=>{if(event.ctrlKey){event.preventDefault();setZoom(zoom+(event.deltaY<0?.1:-.1));}},{passive:false});
$('recipe-name').onchange=()=>change(()=>{recipe.name=$('recipe-name').value;},false);
...
$('export').onclick=()=>{
  try{const compiled=compile(recipe,schema);if(compiled.kind!=='world')throw new Error('Select a Playable world output.');
    ...
    download(new Blob([JSON.stringify(recipe,null,2)+'\n'],{type:'application/json'}),'wasteland.json');
    message('Recipe exported. Save it as maps/wasteland.json, then run ./build.ps1 and ./test.ps1.');
...
$('import-file').onchange=async()=>{
  ...
  try{
    ...
    change(()=>{recipe=imported;selected=recipe.output;pending=null;pinnedSettings=null;$('view').value='final';});
